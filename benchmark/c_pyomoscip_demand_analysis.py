...
def redistribute_demand(demand_profile, method="variance", factor=1.5):
    total_demand = np.sum(demand_profile)
    mean_demand = np.mean(demand_profile)
    
    if method == "variance":
        new_profile = (demand_profile - mean_demand) * factor + mean_demand

    new_profile = new_profile * (total_demand / np.sum(new_profile))
    return new_profile

new_demand_profile = redistribute_demand(demand_profile, method="variance", factor=1.2)
print("Redistribuição Variância:", new_demand_profile)

print(sum(demand_profile) == sum(new_demand_profile))

dispatch_SCIP = DispatchValvePointRampLimits(parameters, new_demand_profile, "scip", 60) #t em s
